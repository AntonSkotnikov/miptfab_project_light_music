...
## Режимы работы
Плата разведена, все компоненты спаяны, источник питания работает. Вот и пришло время написания кода для, собственно, самой светомузыки. Было реализовано два режима работы ленты, а также переключение между ними и регулировка чувствительности микрофона с помощью датчика дальности.

1. `Aurora Borealis`

Наша "рабочая поверхность" состоит из 10 фрагментов ленты разной длины, расположенных своеобразной лесенкой. Первый режим ассоциирует каждый горизонтальный фрагмент с некоторым диапазоном частот. Чем выше расположен участок ленты, тем более высокие частоты входят в диапазон, за который он отвечает. Чем громче звучат частоты того или иного диапазона, тем дальше от середины распространяется свет по соответствующему участку. Образуется пульсирующая картина из сиреневых и сине-зелëных полос, напоминающая северное сияние. `Aurora Borealis` выглядит зрелищно при любом входящем аудиоряде, так как реагирует на большинство частот, которые только могут встретиться в музыке.

2. `Broadway`

Теперь будем рассматривать нашу светодиодную "матрицу" как столбцы. Когда микрофон улавливает достаточное количество высоких и низких частот - фактически, когда в песне звучит бит или яркий акцент - по столбцам пробегает импульс. В зависимости от чëтности столбцов фиолетовые и огненно-оранжевые импульсы бегут в разные стороны, усиливая при этом свою яркость на особенно громкие акценты. Этот режим предназначается преимущественно для электронной музыки, где высокие и низкие частоты звучат отрывисто, создавая тем самым яркие вспышки. Энергия и сочность этого режима навевают ассоциации с нескончаемыми шоу Бродвея.

Руководит сим праздником ультразвуковой датчик дальности. Удерживая 2 секунды руку в его зоне видимости, вы переключите режим. При этом в зависимости от расстояния от руки до датчика выберется чувствительность, с которой чудо-устройство будет реагировать на звук. Чем ниже к датчику рука, тем меньше чувствительность, и тем слабее будут импульсы и пульсации светодиодов; максимальная же реакция реализовывается при дистанции 50 см - а далее датчик перестаëт считывать руку.

## Программная часть
На данный момент тут представлены 4 файла:
...
* `release_build.ino` - финальная версия рабочего кода, с поддержкой двух режимов и взаимодействия с датчиком дальности

## Конечный результат

![фото 1](images/final_result_1.png)
:------------------------------------:
Рис 9. Фото проекта с одного ракурса

![фото 2](images/final_result_2.png)
:------------------------------------:
Рис 10. Фото проекта с другого ракурса
